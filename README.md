...
- **접속이 안 될 때** — 폰 브라우저를 완전히 닫았다 다시 열어보고, 그래도 안 되면 터미널에 표시된 IP로 직접 접속해 보세요. 맥 시스템 설정에서 Python의 네트워크(로컬 네트워크) 접근이 허용돼 있는지도 확인하세요.
  - **같은 와이파이인데도 컴퓨터에선 되고 폰에서만 안 될 때(윈도우)** — 십중팔구 **윈도우 방화벽**이 폰의 접속을 막고 있는 겁니다. 컴퓨터 자체 브라우저는 방화벽을 거치지 않아서 항상 되고, 폰처럼 다른 기기에서 들어오는 접속만 막히는 게 특징입니다.
    1. 서버를 처음 실행했을 때 화면에 **"Windows Defender 방화벽이 이 앱의 일부 기능을 차단했습니다"** 팝업이 떴었는지 떠올려 보세요. 떴었다면 그때 **"취소"**를 눌렀거나 그냥 닫았을 가능성이 높습니다.
    2. 시작 메뉴에서 **"방화벽이 있는 상태로 앱 허용"**(또는 "Windows Defender 방화벽을 통해 앱 허용")을 검색해서 엽니다.
    3. **설정 변경** 버튼을 누른 뒤 목록에서 **Python**을 찾습니다. 없다면 **다른 앱 허용**을 눌러 `python.exe`(보통 `AppData\Local\Programs\Python\...` 안에 있음)를 직접 추가합니다.
    4. Python 항목의 **개인** 체크박스에 체크한 뒤(집 와이파이는 보통 "개인" 네트워크입니다) 저장합니다.
    5. 서버를 다시 실행하고 폰에서 재접속해 보세요.
    - 그래도 안 되면: 네트워크 위치가 "공용"으로 잘못 설정돼 있을 수 있습니다. 설정 → 네트워크 및 인터넷 → Wi-Fi → 현재 연결된 네트워크를 눌러 **네트워크 프로필 유형을 "개인"으로 변경**해 보세요.
